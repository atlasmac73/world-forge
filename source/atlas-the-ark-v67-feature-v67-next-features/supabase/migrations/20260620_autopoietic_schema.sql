-- ============================================================
-- ATLAS v67 — Autopoietic Engine Schema Fix
-- Brings schema_v67.sql's tables into the actual migration chain
-- (that file was never referenced in DEPLOYMENT_CHECKLIST.md's
-- deploy steps, so system_config / build_blueprints /
-- selfbuild_tasks / model_registry / sprint_log were never
-- created against a real deploy) and adds the genesis_cycles
-- columns lib/autopoietic/heartbeat.ts actually writes to
-- (sense_data, interpret_data, mutate_data, simulate_data,
-- promote_data, error_log), which schema.sql never defined.
-- Isaac Brandon Burdette · Atlas Genesis Matrix LLC
-- ============================================================

-- ═══ GENESIS_CYCLES — add columns runHeartbeatTick() needs ═══
alter table public.genesis_cycles
  add column if not exists sense_data jsonb,
  add column if not exists interpret_data jsonb,
  add column if not exists mutate_data jsonb,
  add column if not exists simulate_data jsonb,
  add column if not exists promote_data jsonb,
...
create table if not exists public.build_blueprints (
  ...
  cycle_id uuid references public.genesis_cycles(id) on delete set null,
  title text not null,
  description text,
